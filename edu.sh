...
wget -q -O /usr/local/bin/edu-ssh https://raw.githubusercontent.com/Tarap-Kuhing/main/main/proxy-template.py
chmod +x /usr/local/bin/edu-ssh

# Installing Service
cat > /etc/systemd/system/edussh-nontls.service << END
[Unit]
Description=Ssh Websocket By Tarap-Kuhing
Documentation=/store
...
wget -q -O /usr/local/bin/edu-ovpn https://raw.githubusercontent.com/Tarap-Kuhing/main/main/edu-op.py
...
[Unit]
Description=Ovpn Websocket By Tarap-Kuhing
Documentation=/store
After=network.target nss-lookup.target
